...
class PVgeneral(object):
  ...
  dIk_dT = symbols("\dfrac{\partial~I_k}{\partial~T}", real=True)
  dVk_dT = symbols("\dfrac{\partial~V_k}{\partial~T}", real=True)
  dI_dV  = symbols("\dfrac{\partial~I}{\partial~V}", real=True)
  dIV_dV = symbols("\dfrac{\partial~IV}{\partial~V}", real=True)
  ...
  def get_temperature_coefficients(self, PVModule, Tcoeff):
    return Tcoeff.loc[PVModule][0:].tolist()
