...
class PVfitting_11PFF_CEC(PVfitting):
  ...
  def compile(self, S_ref, T_ref, Ns, n_max=2.5, n_min=0.5, verbose=True):
    # =========================================================
  ...
  def run(self, 
          ...
          Ns, Np, 
          alpha, beta, gamma,
          S_STC, T_STC, Voc_STC, Isc_STC, Vmp_STC, Imp_STC, 
          S_NOCT, T_NOCT, Voc_NOCT, Isc_NOCT, Vmp_NOCT, Imp_NOCT,
          S_LIC, T_LIC, Voc_LIC, Isc_LIC, Vmp_LIC, Imp_LIC,
          # solver params
          n_max=2.0, 
          n_min=0.5, 
          Rs_min=1e-6,
    ...
    assert isinstance(PVModule, str)
    ...
    assert isinstance(S_STC, float)
    assert isinstance(T_STC, float)
    assert isinstance(Voc_STC, float)
    assert isinstance(Isc_STC, float)
    assert isinstance(Vmp_STC, float)
    assert isinstance(Imp_STC, float)

    assert isinstance(S_NOCT, float)
    assert isinstance(T_NOCT, float)
    assert any([isinstance(Voc_NOCT, float), Voc_NOCT is np.nan, Voc_NOCT is None])
    assert any([isinstance(Isc_NOCT, float), Isc_NOCT is np.nan, Isc_NOCT is None])
    assert isinstance(Vmp_NOCT, float)
    assert isinstance(Imp_NOCT, float)

    assert isinstance(S_LIC, float)
    assert isinstance(T_LIC, float)
    assert any([isinstance(Voc_LIC, float), Voc_LIC is np.nan, Voc_LIC is None])
    assert any([isinstance(Isc_LIC, float), Isc_LIC is np.nan, Isc_LIC is None])
    assert isinstance(Vmp_LIC, float)
    assert isinstance(Imp_LIC, float)



    def prediction(mPFF, model):
      # =========== Reference conditions
      [Isc_m, Vsc_m, Imp_m, Vmp_m, Pmp_m, Ioc_m, Voc_m] = mPFF.predict(S_STC, T_STC)
      ...
      [Isc_m, Vsc_m, Imp_m, Vmp_m, Pmp_m, Ioc_m, Voc_m] = mPFF.predict(S_NOCT, T_NOCT)
      ...
      [Isc_m, Vsc_m, Imp_m, Vmp_m, Pmp_m, Ioc_m, Voc_m] = mPFF.predict(S_LIC, T_LIC)
    ...
    self.spec_Tcoeff = [alpha, beta, gamma]
    df_Gen = pd.DataFrame([[alpha, beta, gamma, Ns, Np]], columns=['alpha', 'beta', 'gamma', 'N_s', 'N_p'])
    df_Gen.columns = pd.MultiIndex.from_product([['DataSheet'], df_Gen.columns, ['']])

    # get first point (reference)
    Pmp_STC = Vmp_STC*Imp_STC
    self.meas_ds1 = [S_STC, T_STC, Voc_STC, Isc_STC, Vmp_STC, Imp_STC, Pmp_STC]
    df_STC = pd.DataFrame([[S_STC, T_STC-T0, Voc_STC, Isc_STC, Vmp_STC, Imp_STC, Pmp_STC]], columns=['S', 'T', 'Voc', 'Isc', 'Vpmax', 'Ipmax', 'Pmax'])
    df_STC.columns = pd.MultiIndex.from_product([['DataSheet'], ['STC'], df_STC.columns])
    S_ref, T_ref = self.meas_ds1[:2]

    # get second point (complementary 1)
    Pmp_NOCT = Vmp_NOCT*Imp_NOCT
    self.meas_ds2 = [S_NOCT, T_NOCT, Voc_NOCT, Isc_NOCT, Vmp_NOCT, Imp_NOCT, Pmp_NOCT]
    df_NOCT = pd.DataFrame([[S_NOCT, T_NOCT-T0, Voc_NOCT, Isc_NOCT, Vmp_NOCT, Imp_NOCT, Pmp_NOCT]], columns=['S', 'T', 'Voc', 'Isc', 'Vpmax', 'Ipmax', 'Pmax'])
    df_NOCT.columns = pd.MultiIndex.from_product([['DataSheet'], ['NOCT'], df_NOCT.columns])

    # get second point (complementary 2)
    Pmp_LIC = Vmp_LIC*Imp_LIC
    self.meas_ds3 = [S_LIC, T_LIC, Voc_LIC, Isc_LIC, Vmp_LIC, Imp_LIC, Pmp_LIC]
    df_LIC = pd.DataFrame([[S_LIC, T_LIC-T0, Voc_LIC, Isc_LIC, Vmp_LIC, Imp_LIC, Pmp_LIC]], columns=['S', 'T', 'Voc', 'Isc', 'Vpmax', 'Ipmax', 'Pmax'])
    df_LIC.columns = pd.MultiIndex.from_product([['DataSheet'], ['LIC'], df_LIC.columns])
    ...
    df_DataSheet = pd.concat([df_STC, df_NOCT, df_LIC, df_Gen], axis=1)
    self.compile(S_ref, T_ref, Ns, n_max=n_max, n_min=n_min)
      ...
      try:
        try:
          ...
          try:
            ...
            print()
            print('least_squares')
            df11pff, df_iter = self.get_params(x0, tol=tol)
            df11pff['Module'] = PVModule
            df11pff = df11pff.set_index(['Module'])    
          except Exception as e:
            df11pff = None
        except:
            df11pff = None
    ...
    if isinstance(df11pff, pd.DataFrame):
      ...
      df_DataSheet.index = dfx.index
      df_sum = pd.concat([df_DataSheet, dfx, df_pred11PFF], axis=1)
      df_sum.insert(0, 'Technology', Technology)
      df_sum.insert(1, 'Gamma', self.gammaIV)
      return df_sum.reset_index().set_index(['Technology', 'Module', 'Gamma'])
