#  ..........................\Lib\site-packages\scipy\optimize\_lsq
...
Cauchy step (uba minimizer along -g_h in "hat" space, or -D^2 g in the original
space).
...
def trf(fun, jac, x0, f0, J0, lb, ub, ftol, xtol, gtol, max_nfev, x_scale,
        loss_function, tr_solver, tr_options, verbose, bound_callbacks, weigths):
    ...
    if bound_callbacks is None:
        ...
        return trf_bounds_function(
                fun, jac, x0, f0, J0, bound_callbacks, weigths, ftol, xtol, gtol, max_nfev, x_scale,
                loss_function, tr_solver, tr_options, verbose)
...
def trf_bounds_function(fun, jac, x0, f0, J0, bound_callbacks, weigths, ftol, xtol, gtol, max_nfev,
               x_scale, loss_function, tr_solver, tr_options, verbose):
    x = copy.deepcopy(x0)
    lb, ub = bound_callbacks(x[0])
    lb = np.real(lb)
    ub = np.real(ub)
    f_lb = x<lb
    f_ub = x>ub
    ...
    if loss_function is not None:
        rho = loss_function(f)
        # ===========================
        assert rho.shape[1] == len(weigths)
        for id_w, w in enumerate(weigths):
            rho[0, id_w] *= w
        # ===========================
        cost = 0.5 * np.sum(rho[0])
        J, f = scale_for_robust_loss_function(J, f, rho)
